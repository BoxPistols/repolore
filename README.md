...
**Status:** Alpha. Phases 1–5 and 7 are shipped: four viewpoints (`architecture`, `deps`, `git-history`, `python`), two output formats (Mermaid, DOT), Claude Code skill, GitHub Action, and BYOK Anthropic LLM curation. Go analyzer, SVG output, and the marquee history×architecture fusion are still on the roadmap.
...
Options:
  ...
  --viewpoints <list>     Comma-separated viewpoint IDs (default: architecture)
                          Available: architecture, deps, git-history, python
...
- **Phase 4** *(current)* ✅ — Python analyzer (regex-based; AST upgrade later). Go: planned
- **Phase 5** *(current)* ✅ — DOT renderer. SVG: planned (needs `mmdc` binary)
- **Phase 7** ✅ — GitHub Action (`action.yml` composite action)
- **Phase 2.5** — fuse git-history × architecture into one view (the marquee differentiator)
